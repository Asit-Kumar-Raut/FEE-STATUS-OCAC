...
def main(admin_name, admin_id):
    ...
    lbl_title = Label(root, text="COUNSELORS REGISTRATION LIST", fg="#1e293b", bg=bg_color, font=("Segoe UI", 20, "bold"))
    lbl_title.place(x=480, y=80)

    # Table Column Headers
    Label(root, text="Counselor ID", font=("Segoe UI", 11, "bold"), fg="#1e293b", bg=bg_color).place(x=100, y=140)
    Label(root, text="Name", font=("Segoe UI", 11, "bold"), fg="#1e293b", bg=bg_color).place(x=250, y=140)
    Label(root, text="Username", font=("Segoe UI", 11, "bold"), fg="#1e293b", bg=bg_color).place(x=450, y=140)
    Label(root, text="Contact", font=("Segoe UI", 11, "bold"), fg="#1e293b", bg=bg_color).place(x=650, y=140)
    ...
    if not counselors:
        ...
        for c in counselors:
            ...
            Label(root, text=username, font=("Segoe UI", 10), fg="#1e293b", bg=bg_color).place(x=450, y=y)
            Label(root, text=contact, font=("Segoe UI", 10), fg="#1e293b", bg=bg_color).place(x=650, y=y)
            ...
            btn_accept.bind("<Enter>", lambda e, b=btn_accept: b.config(bg="#059669"))
            btn_accept.bind("<Leave>", lambda e, b=btn_accept: b.config(bg="#10b981"))

            btn_delete = Button(root, text="Delete", fg="white", bg="#ef4444", activebackground="#dc2626", activeforeground="white", font=("Segoe UI", 9, "bold"), bd=0, cursor="hand2", command=make_delete_cmd(c_id))
