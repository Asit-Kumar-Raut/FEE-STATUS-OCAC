...
def main(admin_name, admin_id):
    ...
    lbl_title = Label(root, text="FULLY PAID STUDENT LIST", fg="#1e293b", bg=bg_color, font=("Segoe UI", 20, "bold"))
    lbl_title.place(x=480, y=80)

    # Search Bar
    Label(root, text="Search Fully Paid Student ID:", font=("Segoe UI", 11, "bold"), fg="#1e293b", bg=bg_color).place(x=100, y=155)
    ...
    def search_action():
        ...
        if s_id == "":
            student_list = db.get_students_by_college(admin_name, status="Accepted", paid_status="Fully Paid")
        else:
    ...
    def display_students(student_list):
        ...
        if not student_list:
            lbl_empty = Label(table_frame, text="No fully paid students found yet!⌛", fg="#ef4444", bg=bg_color, font=("Segoe UI", 14, "bold"))
            lbl_empty.place(x=450, y=150)
            ...
            for s in student_list:
                ...
                l5.place(x=700, y=y)
                l6 = Label(table_frame, text=f"₹ {pending:,}", font=("Segoe UI", 10, "bold"), fg="#10b981", bg=bg_color)
                l6.place(x=850, y=y)
    ...
    # Initial load
    initial_list = db.get_students_by_college(admin_name, status="Accepted", paid_status="Fully Paid")
