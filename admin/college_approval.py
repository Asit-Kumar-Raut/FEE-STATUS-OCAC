...
def main(admin_name, admin_id):
    root = Tk()
    root.title("College Approval Panel")
    ...
    def accept_college(cid):
        db.approve_college(cid)
        messagebox.showinfo("Success", f"College ID {cid} has been accepted successfully!🤗")
        root.destroy()
    ...
    lbl_title = Label(root, text="PENDING COLLEGES REGISTRATION LIST", fg="#1e293b", bg=bg_color, font=("Segoe UI", 20, "bold"))
    lbl_title.place(x=420, y=80)
    ...
    colleges = db.get_pending_colleges()

    if not colleges:
        Label(root, text="No pending colleges found.", fg="#64748b", bg=bg_color, font=("Segoe UI", 14, "bold")).place(x=520, y=280)
    else:
        ...
        for col in colleges:
            ...
            Label(root, text=status, font=("Segoe UI", 10, "bold"), fg="#d97706", bg=bg_color).place(x=950, y=y)

            # Accept/Delete functions
            def make_accept_cmd(id_val):
                return lambda: accept_college(id_val)

            def make_delete_cmd(id_val):
                return lambda: delete_college(id_val)

            btn_accept = Button(root, text="Accept", fg="white", bg="#10b981", font=("Segoe UI", 9, "bold"), bd=0, cursor="hand2", command=make_accept_cmd(cid))
            btn_accept.place(x=1080, y=y, width=70, height=25)

            btn_delete = Button(root, text="Delete", fg="white", bg="#ef4444", font=("Segoe UI", 9, "bold"), bd=0, cursor="hand2", command=make_delete_cmd(cid))
            btn_delete.place(x=1160, y=y, width=70, height=25)
