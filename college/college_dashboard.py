...
def main(college_name, college_id):
    root = Tk()
    root.title("College Dashboard")
    root.geometry("1366x768+0+0")
    root.resizable(False, False)
    bg_color = "#fef3c7" # Amber theme for college
    ...
    def show_counselors():
        ...
        # Pass college_name and college_id as the dashboard parameters
        counciler_approval.main(college_name, college_id)

    def show_students():
        root.destroy()
        from admin import student_approval_option
        student_approval_option.main(college_name, college_id)

    def show_academic_fee():
        root.destroy()
        from admin import academic_fee
        academic_fee.main(college_name, college_id)

    def show_feedbacks():
        root.destroy()
        from admin import feedbackform_student
        feedbackform_student.main(college_name, college_id)

    def show_counselor_feedbacks():
        root.destroy()
        from admin import counselor_feedbacks
        counselor_feedbacks.main(college_name, college_id)

    def change_password():
        root.destroy()
        import admin.update_password_admin as update_password_admin
        # We can reuse the update password admin interface but it will update the college password!
        # We will adjust the update password logic to handle if it's college or admin.
        update_password_admin.main(college_name, college_id, is_college=True)

    # Header bar
    header_frame = Frame(root, bg="#78350f") # Deep amber header
    header_frame.place(x=0, y=0, width=1366, height=60)

    lbl_admin = Label(header_frame, text=f"🏛️ COLLEGE PROFILE: {college_name} (ID: {college_id})", fg="#fef3c7", bg="#78350f", font=("Segoe UI", 12, "bold"))
    ...
    btn_reset = Button(root, text="🔄 Reset Password", fg="white", bg="#d97706", activebackground="#b45309", activeforeground="white", font=("Segoe UI", 10, "bold"), bd=0, cursor="hand2", command=change_password)
    btn_reset.place(x=30, y=690, width=140, height=40)
    btn_reset.bind("<Enter>", lambda e: btn_reset.config(bg="#b45309"))
    btn_reset.bind("<Leave>", lambda e: btn_reset.config(bg="#d97706"))

    # Title
    title_label = Label(root, text="COLLEGE ADMINISTRATIVE CONTROL", fg="#78350f", bg=bg_color, font=("Segoe UI", 24, "bold"))
    title_label.place(x=420, y=110)

    # 1st TILE: Counselor approvals
